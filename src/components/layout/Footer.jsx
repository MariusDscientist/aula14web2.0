...
const Footer = ({ sections, siteInfo }) => {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          <div className="text-left space-y-4">
             <div className="flex items-center gap-3">
               <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center font-bold text-xl">
                 {siteInfo?.shortName || '14'}
               </div>
               <span className="text-2xl font-bold">{siteInfo?.name || 'Fundación Aula 14'}</span>
             </div>
             <p className="text-gray-400 max-w-xs">
               {siteInfo?.slogan || 'Transformando el tejido social a través del arte, la cultura y la gestión del patrimonio en el territorio.'}
             </p>
          </div>
          ...
          <div className="text-left">
             ...
             <p className="text-gray-400 mb-4 text-sm">{siteInfo?.newsletterText || 'Suscríbete para recibir noticias de impacto.'}</p>
        ...
        <div className="pt-8 border-t border-gray-800 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-500 text-sm">
          <p>© {new Date().getFullYear()} {siteInfo?.name || 'Fundación Aula 14'}. Todos los derechos reservados.</p>
